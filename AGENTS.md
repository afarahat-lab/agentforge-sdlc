# AGENTS.md — Gestalt platform repository
...
Gestalt is a self-hosted agent-first software development platform. It is a monorepo containing:
...
2. **Agents never import from other agents.** All inter-agent communication goes through the message queue via `@gestalt/core`.
3. **Adapters implement the repository interface defined in `@gestalt/core`.** No adapter-specific code outside the adapter package.
4. **No direct database calls outside adapter packages.** All data access through the repository pattern.
5. **No LLM provider SDK imported outside `@gestalt/core/llm`.** Provider abstraction lives in core only.
...
- **Environment variables** accessed only through `@gestalt/core/config`, never `process.env` directly.
...
- Call an LLM provider directly — always use `@gestalt/core/llm`
